        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                if (!string.IsNullOrEmpty(imgpath))
                {
                    ...
                    string LocalDir = $@"C:\Users\{Account_Name}\AppData\Local\Achi Cinema\";

                    if (!Directory.Exists(LocalDir))
                    {
                        Directory.CreateDirectory(LocalDir);

                        string ImgDir = Path.Combine(LocalDir, "Diractor_Images");
                        ...
                        string TargetDir = Path.Combine(ImgDir, Path.GetFileName(imgpath));
                        File.Copy(imgpath, TargetDir, true);
                        r_img.Image = Image.FromFile(TargetDir);

                        connection.Open();
                        SqlCommand register = new SqlCommand("INSERT INTO Diractor_Info(AD,SOYAD,DOGUM,CINSIYET,RESIM,BIO) VALUES(@name,@surname,@bdate,@gender,@img,@bio)", connection);
                        Functions.Register(register, r_name, r_surname, r_date, r_gender, ImgDir, r_bio);
